namespace BenchmarkDotNet.Analyzers.Tests.Fixtures
{
    ...
    public class ValueTupleDouble<T1, T2> : IXunitSerializable
    {
        ...
        public static implicit operator ValueTupleDouble<T1, T2>((T1, T2) valueTupleDouble) => new() { Value1 = valueTupleDouble.Item1, Value2 = valueTupleDouble.Item2 };

        public override string ToString() => Value1 == null || Value2 == null ? "<empty>" : $"{Value1} · {Value2}";
    }
}
