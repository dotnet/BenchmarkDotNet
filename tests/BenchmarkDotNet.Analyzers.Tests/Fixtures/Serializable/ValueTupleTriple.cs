namespace BenchmarkDotNet.Analyzers.Tests.Fixtures
{
    ...
    public class ValueTupleTriple<T1, T2, T3> : IXunitSerializable
    {
        ...
        public static implicit operator ValueTupleTriple<T1, T2, T3>((T1, T2, T3) valueTupleTriple) => new() { Value1 = valueTupleTriple.Item1, Value2 = valueTupleTriple.Item2, Value3 = valueTupleTriple.Item3 };

        public override string ToString() => Value1 == null || Value2 == null || Value3 == null  ? "<empty>" : $"{Value1} · {Value2} · {Value3}";
    }
}
