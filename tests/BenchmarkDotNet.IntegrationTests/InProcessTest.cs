    ...
    public class InProcessTest : BenchmarkTestExecutor
    {
        ...
        private void TestInvoke(BenchmarkAction benchmarkAction, int unrollFactor, bool isIdle, object expectedResult, ref int counter)
        {
            try
            {
                ...
                if (isIdle)
                {
                    awaitHelper.GetResult(benchmarkAction.InvokeSingle());
                    Assert.Equal(0, counter);
                    awaitHelper.GetResult(benchmarkAction.InvokeUnroll(0, clock));
                    Assert.Equal(0, counter);
                    awaitHelper.GetResult(benchmarkAction.InvokeUnroll(11, clock));
                    Assert.Equal(0, counter);
                }
                else
                {
