...
namespace BenchmarkDotNet.Tests.Mocks
{
    public static class MockFactory
    {
        ...
        private static BenchmarkReport CreateReport(BenchmarkCase benchmarkCase, int n, double nanoseconds)
        {
            ...
            var executeResult = new ExecuteResult(0, default, Array.Empty<string>(), new[] { $"// Runtime=extra output line" });
        ...
        {
            var buildResult = BuildResult.Success(GenerateResult.Success(ArtifactsPaths.Empty, Array.Empty<string>()), string.Empty);
            var executeResult = new ExecuteResult(0, default, Array.Empty<string>(), Array.Empty<string>());
