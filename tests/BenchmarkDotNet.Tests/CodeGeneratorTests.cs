...
namespace BenchmarkDotNet.Tests
{
    public class CodeGeneratorTests
    {
        ...
        public void UsingStatementsInTheAutoGeneratedCodeAreProhibited()
        {
            ...
            var benchmark = BenchmarkCase.Create(target, Job.Default, new ParameterInstances(Array.Empty<ParameterInstance>()), ManualConfig.CreateEmpty().CreateFinalConfig());

            var generatedSourceFile = CodeGenerator.Generate(new BuildPartition(new[] { new BenchmarkBuildInfo(benchmark, ManualConfig.CreateEmpty().CreateFinalConfig(), 0) }, BenchmarkRunner.DefaultResolver));
