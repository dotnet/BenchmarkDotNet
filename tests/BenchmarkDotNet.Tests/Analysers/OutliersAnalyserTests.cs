    ...
    public class OutliersAnalyserTests
    {
        ...
        public void RangeMessageTest(int lowerOutliers, int upperOutliers, string expectedMessage)
        {
            ...
            output.WriteLine("Values   : " +
                             string.Join(", ", values.Take(5).Select(x =>  TimeValue.FromNanoseconds(x).ToString(cultureInfo, "N2"))) +
                             ", ..., " +
                             string.Join(", ", values.Skip(values.Count - 5).Select(x => TimeValue.FromNanoseconds(x).ToString(cultureInfo, "N2"))));
