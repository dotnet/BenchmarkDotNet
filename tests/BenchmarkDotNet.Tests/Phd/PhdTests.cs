...
public class PhdTests(ITestOutputHelper output)
{
            ...
            "sort01", new PhdAttributes
            {
                Meta = new PhdMeta
                {
                    Table = new PhdTableConfig
                    {
                        ...
                        SortPolicies =
                        [
                            new PhdSortPolicy("=average", PhdSortDirection.Descending)
                        ]
                    }
                }
            }.ToPerfEntry().Add(Benchmark("Foo", "10ms"), Benchmark("Bar", "20ms"))
        }
    };

    [UsedImplicitly] public static TheoryData<string> EntryDataKeys = TheoryDataHelper.Create(EntryDataMap.Keys);

    private static PhdEntry Root() => new PhdAttributes
    {
        Meta = new PhdMeta
        {
            Table = new PhdTableConfig
        ...
        Host = MockHostEnvironmentInfo.GetCurrent().ToPhd()
    }.ToPerfEntry();

    private static PhdEntry Job(RuntimeMoniker? runtime = null, Jit? jit = null, int? affinity = null) => new PhdAttributes
    {
        Job = new PhdJob
        {
    ...
    private static PhdEntry Benchmark(string name, params string[] metrics) => new PhdAttributes
    {
        Benchmark = new BdnBenchmark { Type = "Bench", Method = name }
    }.ToPerfEntry().AddMetrics(metrics);
