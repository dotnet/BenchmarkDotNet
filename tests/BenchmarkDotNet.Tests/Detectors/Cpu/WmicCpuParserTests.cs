...
public class WmicCpuParserTests
{
    [Fact]
    public void EmptyTest()
    {
        var parser = WmicCpuParser.ParseOutput(string.Empty);
    ...
    {
        var parser = WmicCpuParser.ParseOutput("malformedkey=malformedvalue\n\nmalformedkey2=malformedvalue2");
        Assert.Null(parser.ProcessorName);
        Assert.Null(parser.PhysicalProcessorCount);
        Assert.Null(parser.PhysicalCoreCount);
        Assert.Null(parser.LogicalCoreCount);
...
";
        var parser = WmicCpuParser.ParseOutput(cpuInfo);
        ...
        Assert.Equal(2400 * Frequency.MHz, parser.GetMaxFrequency());
    }
    ...
    public void RealTwoProcessorEightCoresWithWmicBugTest()
    {
        ...
        Assert.Equal(16, parser.PhysicalCoreCount);
        Assert.Equal(32, parser.LogicalCoreCount);
        Assert.Equal(3111 * Frequency.MHz, parser.GetMaxFrequency());
    }
...
";
        ...
        Assert.Equal(2500 * Frequency.MHz, parser.GetMaxFrequency());
    }
}