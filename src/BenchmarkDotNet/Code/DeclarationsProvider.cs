...
namespace BenchmarkDotNet.Code
{
    ...
    {
        protected const string AggressiveOptimizationAttribute = "[global::System.Runtime.CompilerServices.MethodImpl(global::BenchmarkDotNet.Portability.CodeGenHelper.AggressiveOptimizationOption)]";
        ...
        private void Replace(SmartStringBuilder smartStringBuilder, MethodInfo? method, string replaceModifiers, string replaceImpl, bool isGlobalCleanup)
        {
            ...
            {
                modifier = "async";
                impl = $"await {GetMethodPrefix(Descriptor.WorkloadMethod)}.{method.Name}();";
            }
            else
            {
                modifier = string.Empty;
                impl = $"""
                {GetMethodPrefix(Descriptor.WorkloadMethod)}.{method.Name}();
    ...
    internal class SyncDeclarationsProvider(BenchmarkCase benchmark) : DeclarationsProvider(benchmark)
    {
        ...
        protected override SmartStringBuilder ReplaceCore(SmartStringBuilder smartStringBuilder)
        {
            ...
            string coreImpl = $$"""
            {{AggressiveOptimizationAttribute}}
    ...
    internal class AsyncDeclarationsProvider(BenchmarkCase benchmark) : DeclarationsProvider(benchmark)
    {
        protected override string PrependExtraGlobalCleanupImpl(string impl)
            => $"""
            this.__workloadContinuerAndValueTaskSource?.Complete();
                        {impl}
            """;
        ...
        protected override SmartStringBuilder ReplaceCore(SmartStringBuilder smartStringBuilder)
        {
            ...
            {{AggressiveOptimizationAttribute}}
                    ...
                    {
                        this.__invokeCount = invokeCount;
                        this.__clock = clock;
                        if (this.__workloadContinuerAndValueTaskSource == null)
                        {
                            this.__workloadContinuerAndValueTaskSource = new();
                            this.__StartWorkload();
                        }
                        return this.__workloadContinuerAndValueTaskSource.Continue();
                    }
                    ...
                    {{AggressiveOptimizationAttribute}}
                    private async {{workloadCoreReturnType.GetCorrectCSharpTypeName()}} WorkloadCore()
                    {
                        try
                        {
                            while (true)
                            {
                                await this.__workloadContinuerAndValueTaskSource;
                                if (this.__workloadContinuerAndValueTaskSource.IsCompleted)
                                {
                                    {{finalReturn}}
                                }

                                {{typeof(StartedClock).GetCorrectCSharpTypeName()}} startedClock = {{typeof(ClockExtensions).GetCorrectCSharpTypeName()}}.Start(this.__clock);
                                while (--this.__invokeCount >= 0)
                                {
                                ...
                                this.__workloadContinuerAndValueTaskSource.SetResult(startedClock.GetElapsed());
                            }
                        }
                        catch (global::System.Exception e)
                        {
                            __workloadContinuerAndValueTaskSource.SetException(e);
                            {{finalReturn}}
                        }
                    }
            """;
            ...
            return smartStringBuilder
                .Replace("$ExtraFields$", $"""
                    private {typeof(WorkloadContinuerAndValueTaskSource).GetCorrectCSharpTypeName()} __workloadContinuerAndValueTaskSource;
                            private global::Perfolizer.Horology.IClock __clock;
                            private long __invokeCount;
                    """)
                .Replace("$DisassemblerEntryMethodImpl$", GetWorkloadMethodCall(GetPassArgumentsDirect()))
                .Replace("$CoreImpl$", coreImpl);
        }
