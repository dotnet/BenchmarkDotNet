...
namespace BenchmarkDotNet.Code
{
    internal abstract class DeclarationsProvider
    {
        ...
        public virtual string GetAsyncBenchmarkRunnerFieldDeclarations(BenchmarkId id) => null;
    ...
    internal class AsyncDeclarationsProvider : DeclarationsProvider
    {
        ...
        protected override Type OverheadMethodReturnType => typeof(EmptyAwaiter); // we return this simple type because creating bigger ValueType could take longer than benchmarked method itself

        private string GetRunnableName(BenchmarkId id) => $"BenchmarkDotNet.Autogenerated.Runnable_{id}";

        private (string workloadRunnerTypeName, string overheadRunnerTypeName) GetAsyncBenchmarkRunnerTypeNames(BenchmarkId id)
        {
            ...
            string awaiterTypeName = genericArguments[1].GetCorrectCSharpTypeName();
            ...
            return (
                $"BenchmarkDotNet.Engines.AsyncWorkloadRunner<{runnableName}.WorkloadFunc, {consumerTypeName}, {WorkloadMethodReturnTypeName}, {awaiterTypeName}{appendResultType}>",
                $"BenchmarkDotNet.Engines.AsyncOverheadRunner<{runnableName}.OverheadFunc, {consumerTypeName}, {WorkloadMethodReturnTypeName}, {awaiterTypeName}{appendResultType}>"
            );
        }

        public override string GetAsyncBenchmarkRunnerFieldDeclarations(BenchmarkId id)
        {
            var (workloadRunnerTypeName, overheadRunnerTypeName) = GetAsyncBenchmarkRunnerTypeNames(id);

            return $"private readonly {workloadRunnerTypeName} __asyncWorkloadRunner;" + Environment.NewLine
         + $"        private readonly {overheadRunnerTypeName} __asyncOverheadRunner;";
        }
        ...
        public override string GetInitializeAsyncBenchmarkRunnerFields(BenchmarkId id)
        {
            ...
            return $"__asyncWorkloadRunner = new {workloadRunnerTypeName}(new {runnableName}.WorkloadFunc(this));"
