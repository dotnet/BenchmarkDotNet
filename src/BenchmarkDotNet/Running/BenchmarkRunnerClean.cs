            ...
            using (var streamLogger = new StreamLogger(GetLogFileStreamWriter(benchmarkRunInfos, logFilePath)))
            {
                ...
                try
                {
                    ...
                    foreach (var benchmarkRunInfo in supportedBenchmarks) // we run them in the old order now using the new build artifacts
                    {
                        ...
                        if (benchmarkRunInfo.Config.Options.IsSet(ConfigOptions.JustBuild))
                                   ...
                                   ref int benchmarksToRunCount)
        {
            ...
            using (var powerManagementApplier = new PowerManagementApplier(logger))
            {
                ...
                for (int i = 0; i < benchmarks.Length && !stop; i++)
                {
                    ...
                    {
                        if (config.Options.IsSet(ConfigOptions.JustBuild))
                        {
                            if (File.Exists(buildResult.ArtifactsPaths.ExecutablePath))
                            {
                                logger.WriteLineHeader($"// The produced executable can be found at '{buildResult.ArtifactsPaths.ExecutablePath}'.");
                            }
                            else
                            {
                                logger.WriteLineHeader($"// Please search the build output printed above for output path of BenchmarkDotNet.Autogenerated project.");
                            }
                            stop = true;
                            break;
                        }
