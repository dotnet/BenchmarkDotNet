...
namespace BenchmarkDotNet.Toolchains.CsProj
{
        ...
        : DotNetCliGenerator(settings, isNetCore)
    {
        ...
        protected async ValueTask GatherReferencesAsync(BuildPartition buildPartition, ArtifactsPaths artifactsPaths, ILogger logger, CancellationToken cancellationToken)
        {
            ...
            // The gatherer builds under the same assembly name as the benchmark program, so every part of
            // its own output would collide with what this project is about to produce.
            foreach (var gathererOutput in Directory.GetFiles(binDirectory, $"{artifactsPaths.ProgramName}.*"))
                File.Delete(gathererOutput);
            ...
            // A .NET Framework exe project has no dll next to its exe, so both extensions have to be
            // referenced for the benchmarks themselves to be reachable.
            foreach (var assemblyFile in Directory.GetFiles(binDirectory)
                    ...
                    || file.EndsWith(".exe", StringComparison.OrdinalIgnoreCase)))
            {
                itemGroup.Add(new XElement("Reference",
                    ...
                    // The whole directory is copied below, so the reference must not copy anything itself.
                    new XElement("Private", "false")
                    // TODO: Add Aliases here for extern alias #2289
                ));
            }

            // A Reference carries the assembly and nothing else, while the benchmark also needs whatever
            // else its project produced: native libraries, satellite assemblies, content files. The gatherer
            // already built all of it, so copy the directory rather than guessing which parts are needed.
            itemGroup.Add(new XElement("None",
                new XAttribute("Include", Path.Combine(binDirectory, "**", "*")),
