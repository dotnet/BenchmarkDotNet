...
partial class RunnableEmitter
{
    ...
    private sealed class AsyncCoreEmitter(BuildPartition buildPartition, ModuleBuilder moduleBuilder, BenchmarkBuildInfo benchmark) : RunnableEmitter(buildPartition, moduleBuilder, benchmark)
    {
        ...
        {
            // Allocate the source and start the async workload loop after the user's GlobalSetup.
            // The loop parks at its first await before the engine begins iterating, so the
            // per-iteration WorkloadActionNoUnroll path is branchless and allocation-free.
        ...
        protected override void EmitExtraGlobalCleanup(ILGenerator ilBuilder, LocalBuilder? thisLocal)
        {
            ...
            // The source is allocated unconditionally in __GlobalSetup, so no null check is needed.
            if (thisLocal != null)
            {
                ilBuilder.EmitLdloc(thisLocal);
            }
