...
namespace BenchmarkDotNet.Toolchains.Wasm
{
    public class WasmExecutor(WasmSettings settings) : IExecutor
    {
        ...
        private async ValueTask<bool> ProbeWebSocketSupportAsync(BenchmarkCase benchmarkCase, ArtifactsPaths artifactsPaths, IResolver resolver, CancellationToken cancellationToken)
        {
        ...
        private async ValueTask<ProcessListener> CreateProcessListenerAsync(
        ...
        private async ValueTask<ExecuteResult> Execute(Process process, BenchmarkCase benchmarkCase, AsyncProcessOutputReader processOutputReader,
