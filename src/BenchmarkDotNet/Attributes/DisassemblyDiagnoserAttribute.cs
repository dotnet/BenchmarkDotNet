...
namespace BenchmarkDotNet.Attributes
{
    ...
    public class DisassemblyDiagnoserAttribute : Attribute, IConfigSource
    {
        ...
        public DisassemblyDiagnoserAttribute(bool printAsm = true, bool printSource = false, bool printPrologAndEpilog = false, int recursiveDepth = 1, bool printDiff = false)
        {
            Config = ManualConfig.CreateEmpty().AddDiagnoser(
                DisassemblyDiagnoser.Create(
                    new DisassemblyDiagnoserConfig(printAsm, printSource, printPrologAndEpilog, recursiveDepth, printDiff)));
        }
