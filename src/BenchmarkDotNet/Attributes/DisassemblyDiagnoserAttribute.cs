...
namespace BenchmarkDotNet.Attributes
{
    ...
    public class DisassemblyDiagnoserAttribute : Attribute, IConfigSource
    {
        ...
        public DisassemblyDiagnoserAttribute(bool printAsm = true, bool printSource = false, int recursiveDepth = 1, bool printDiff = false)
        {
            Config = ManualConfig.CreateEmpty().AddDiagnoser(
                DisassemblyDiagnoser.Create(
                    new DisassemblyDiagnoserConfig(printAsm, printSource, recursiveDepth, printDiff)));
        }
