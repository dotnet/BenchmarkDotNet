...
{
    internal abstract class EngineWarmupStage(IterationMode iterationMode, long invokeCount, int unrollFactor, FrozenEngineParameters parameters) : EngineStage(IterationStage.Warmup, iterationMode, parameters)
    {
        private const int MinOverheadIterationCount = 4;
        internal const int MaxOverheadIterationCount = 10;

        internal static EngineWarmupStage GetOverhead(long invokeCount, int unrollFactor, FrozenEngineParameters parameters)
        ...
        internal static EngineWarmupStage GetWorkload(RunStrategy runStrategy, long invokeCount, int unrollFactor, FrozenEngineParameters parameters)
        {
    ...
    internal sealed class EngineWarmupStageAuto(IterationMode iterationMode, int minIterationCount, int maxIterationCount, long invokeCount, int unrollFactor, FrozenEngineParameters parameters)
    ...
    internal sealed class EngineWarmupStageSpecific(int maxIterationCount, IterationMode iterationMode, long invokeCount, int unrollFactor, FrozenEngineParameters parameters)
        : EngineWarmupStage(iterationMode, invokeCount, unrollFactor, parameters)
    {
        internal override List<Measurement> GetMeasurementList() => new(maxIterationCount);
