...
namespace BenchmarkDotNet.Extensions
{
    public static class StatisticsExtensions
    {
        ...
        public static string ToTimeStr(this Statistics s, Encoding encoding , TimeUnit unit = null, bool calcHistogram = false)
        {
            ...
            builder.AppendLine($"Mean = {s.Mean.ToTimeStr(unit, encoding: encoding)}, StdErr = {s.StandardError.ToTimeStr(unit, encoding: encoding)} ({errorPercent}%); N = {s.N}, StdDev = {s.StandardDeviation.ToTimeStr(unit, encoding: encoding)}");
            builder.AppendLine($"Min = {s.Min.ToTimeStr(unit, encoding: encoding)}, Q1 = {s.Q1.ToTimeStr(unit, encoding: encoding)}, Median = {s.Median.ToTimeStr(unit, encoding: encoding)}, Q3 = {s.Q3.ToTimeStr(unit, encoding: encoding)}, Max = {s.Max.ToTimeStr(unit, encoding: encoding)}");
            builder.AppendLine($"IQR = {s.InterquartileRange.ToTimeStr(unit, encoding: encoding)}, LowerFence = {s.LowerFence.ToTimeStr(unit, encoding: encoding)}, UpperFence = {s.UpperFence.ToTimeStr(unit, encoding: encoding)}");
            builder.AppendLine($"ConfidenceInterval = {s.ConfidenceInterval.ToTimeStr(encoding, unit)}, Margin = {ci.Margin.ToTimeStr(unit, encoding: encoding)} ({ciMarginPercent}% of Mean)");
