...
namespace BenchmarkDotNet.Diagnosers
{
    internal class WindowsDisassembler
    {
        ...
        internal DisassemblyResult Dissasemble(DiagnoserActionParameters parameters)
        {
            ...
            bool needsJitting = parameters.Benchmark.Job.ResolveValue(Jobs.RunMode.RunStrategyCharacteristic, EngineResolver.Instance).NeedsJitting();

            var errors = ProcessHelper.RunAndReadOutput(
                GetDisassemblerPath(parameters.Process, parameters.Benchmark.Job.Env.Platform),
                BuildArguments(parameters, resultsPath, needsJitting));
        ...
        private string BuildArguments(DiagnoserActionParameters parameters, string resultsPath, bool needsJitting)
            => new StringBuilder(200)
                .Append(parameters.Process.Id).Append(' ')
                .Append(needsJitting ? "BenchmarkDotNet.Autogenerated.Runnable" : $"\"{parameters.Benchmark.Target.Type.FullName}\"").Append(' ')
                .Append(needsJitting ? DisassemblerConstants.DiassemblerEntryMethodName : $"\"{parameters.Benchmark.Target.Method.Name}\"").Append(' ')
