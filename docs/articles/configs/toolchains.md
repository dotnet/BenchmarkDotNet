...
If you go to `D:\projects\performance\src\benchmarks\micro\.bdn\Job-KRLVKQ`, you can see the generated project file (named `BenchmarkDotNet.Autogenerated.csproj`), code (`Job-KRLVKQ.cs`) and find the native executable (in the `p` subfolder). Example:

```cmd
cd D:\projects\performance\src\benchmarks\micro\.bdn\Job-KRLVKQ
