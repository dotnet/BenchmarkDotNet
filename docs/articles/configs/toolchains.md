...
If you go to `D:\projects\performance\src\benchmarks\micro\.bdn\1`, you can see the generated project file (named `gen.csproj`), code (`gen.cs`) and find the native executable (in the `p` subfolder). Example:

```cmd
cd D:\projects\performance\src\benchmarks\micro\.bdn\1
cat .\gen.csproj
```
